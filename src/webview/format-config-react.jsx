...
const stateElement = document.getElementById('format-config-state');
const initialState = stateElement ? JSON.parse(stateElement.textContent || '{}') : {};
const spaceOptions = ['无效', '添加空格', '删除空格'];

function createChoiceLabelId(index) {
  return `choice-${index}`;
}

function App() {
  const [choices, setChoices] = useState(() => Array.isArray(initialState.choices) ? initialState.choices : []);
