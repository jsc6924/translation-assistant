...
这是一款独立的文字游戏汉化专用编辑器，实现了dltxt的核心基础功能（文本与术语高亮，一部分快捷键），下载即用无需安装（需要Windows10以上）。

如果你想在手机上使用，现在还支持安卓版，功能与Windows版几乎一样。

如果你需要支持更多的格式，使用更多的功能，请使用vscode插件版。
...
不管你的格式是哪一种，原则上都需要识别格式才能使用。
最新版本会在打开或重启vscode时自动识别格式（如果当前工作区格式未设置），不过这种识别准确性不高，依然建议使用自动设置向导进行识别。
...
4. 如果设置成功，此时你会看到文本中的原文、译文会被高亮成不同颜色。
...
#### 编辑限制
默认设置下，被识别为原文的行无法编辑，并且无法删除或插入行。可以在窗口底部的状态栏启动、关闭限制编辑模式。


## 语法高亮
  区分人名栏，原文，译文。需要识别格式。（如果选择Theme：DLTXT Dark+/Light+，会对部分格式生效，不过该功能已经弃用，请识别格式）
  ![highlight](https://github.com/jsc6924/translation-assistant/blob/master/imgs/highlight.png?raw=true)
...
#### 交换原文行与译文行
在编辑器的右键菜单中可找到该命令。用于修复原文和译文反了的问题。
...
#### 文件浏览器
这里可以浏览当前工作区的文本文件。点击文本可打开预览。点击替换可执行替换。点击替换目录会批量替换该目录下的所有文本。

#### 差异预览
在这里可预览替换规则执行后的文本。在点击替换按钮之前，文本不会被实际替换。
...
### 全文本重复段分析
在资源管理器中，右键一个文件夹，点`文本重复项分析`，即可显示文本中所有重复段，按长度从长到短排序。目前仅支持完全匹配的文本。
...
## 术语库
SimpleTM 已从单一术语库升级为集术语、人称、组员、进度、文本及配置管理为一体的综合汉化统筹平台。详细功能说明请参考[这个文档](README_Simpletm.md)
### 简介
...
（目前也支持UI，在文件浏览器的右键菜单里可以找到dlbuild UI，使用方法一样）
```yaml
extract: #配置提取操作
  input:
...
## Release Notes
#### 4.12 (2026/09/10)
- dlbuild/dltransform有了UI
#### 4.11 （2026/09/08）
- 取消默认格式，改成插件加载时自动执行一次无交互的自动识别
- 右键菜单添加交换原文行和翻译行的命令
#### 4.10 (2026/09/01)
- 右键菜单添加清空光标后的译文行
#### 4.9.1 (2026/08/05)
- 修复复制的相似句子的翻译中换行符没有escape的问题
- 修复括号中的换行符后的空格警告
#### 4.9  （2026/07/24）
- 优化换行符的显示，默认以特殊形式显示换行符。
#### 4.8  （2026/07/19）
- UI可设置跳过批处理的路径（写入.gitignore）
- 自动识别格式可识别出现在行中的人名格式
#### 4.7  （2026/06/02）
- 支持动态高亮原文译文人名
- 优化限制编辑模式
