...
- 然后，使用vscode打开你的项目**所在的文件夹**，不要单独打开一个文件。每个项目都需要使用一个不同的窗口打开。换句话说，不要用同一个窗口打开格式不同的文本。
...
### 段落格式
段落格式的定义为，需要通过上下文才能判断当前行是原文还是译文的格式。文本中可以见到重复出现的格式相同的段落，当前行是原文还是译文需要根据其在当前段落中的相对位置才能判断。
...
「译文2」[rt2]  
```
这种格式原文译文前都没有标签，因此只能通过段落中的相对位置判断。目前段落格式只能通过手动设置使用。
