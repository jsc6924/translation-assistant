...
在翻译文本时可使用键盘快捷键代替鼠标操作（注：可以在vscode设置中更改默认快捷键绑定）
  ...
  - `Alt + Backspace` 删除光标后的译文直至特定字符
  - `Alt + Delete` 删除光标后的译文直至句尾（不包括句尾引号）
  - `Alt + u` 光标移动到行首
  - `Alt + i` 光标移动到当前短句句首
  - `Alt + o` 光标移动到当前短句句尾
  - `Alt + p` 光标移动到行尾
  - `Alt + [` 光标向左移动一格，当遇到省略号时移动两次
  - `Alt + ]` 光标向右移动一格，，当遇到省略号时移动两次
...
#### 侧边栏
从2.24版本开始添加了dltxt的侧边栏，连接SimpleTM以后可以在侧边栏内查看当前项目所有专有名词。点击可更改释义（同右键菜单）。
