...
#### 编辑限制
默认设置下，被识别为原文的行无法编辑，并且无法删除或插入行。可以在设置向导或在vscode设置中搜索strictEditing关闭此功能
...
如果想恢复不再显示警告的汉字，可使用“命令与设置”->“错误与警告”->“清除常用汉字警告白名单”
...
#### 检测错别字
  ...
  3. 在左侧侧边栏找到dltxt图标，点击，在`命令与设置`中找到`百度智能云API`，填写百度智能云的`Access Key`和`Serect Key`（输入后需要按回车）
...
#### 设置文本格式规范
在设置向导中选择`文本格式规范`，可以快速设置文本的格式规范。
![format-config](https://github.com/jsc6924/translation-assistant/blob/master/imgs/format-config.png?raw=true)

选择“上传当前工作区设置到远程术语库”可以推送设置给其他组员。（需要你拥有该术语库的管理员权限）

可以在`dltxt.config.ignoredVSCodeSettingsForSync`中设置需要忽略的配置项（即不上传到远程服务器的配置项）
...
1. 在文本内的右键，选择DLTXT->批量检查译文（或点击 侧边栏->命令与设置->错误与警告->批量检查译文）
2. 按照提示选择范围即可

如果选择批量检查译文并生成报告，那同时还会生成文本形式的报告，可以保存。

想清除警告时，可使用 侧边栏->命令与设置->错误与警告->清除所有警告
...
### 术语库使用
1. 设置好至少一个术语库后，打开一个文本，选中你想同步的词（原文），在右键菜单中，选择`添加/更新术语库词条`
2. 如果想更改释义，选中文本，在右键菜单中，同样选择`添加/更新术语库词条`，输入译文并回车即可。如果想删除词条，则什么都不输入直接回车。
  - 如果一个术语可能存在多个翻译，请使用斜杠`/`分割
3. 可以使用侧边栏的UI复制释义，也可以编辑、删除、搜索词条（在词条上右键）。
...
1. 指定称呼人的规则
2. 默认规则
3. 在默认规则中找不到被称呼人时，在指定称呼人规则中查找所有可能的匹配

### 共享vscode设置
由一个熟悉vscode设置的人事先设置好当前项目需要的设置(`.vscode/setting.json`)，然后在SimpleTM网页上传。
所有术语库使用者在同步术语库的时候也会同时同步共享的vscode设置

如果不想同步，可以搜索`dltxt.config.autoSyncVSCodeSettings`关闭此功能
...
- 添加与更新术语库词条功能合并
- 整合功能至命令与设置中面板
- 将部分右键菜单功能从二级菜单移动到一级菜单
- 增加搜索术语库词条功能
- 如果术语库词条的释义被斜杠分割，则在ctrl+数字快捷键时可以选择插入的释义
#### 3.44  (2025/10/21)
- 能在自动文本识别向导中填写换行符
- 能使用快捷键切换换行符显示
#### 3.43  (2025/10/17)
